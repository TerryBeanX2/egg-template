...
$(document).ready(function () {
    ...
    // ===== Range Slider =====//
    if ($('#prc-rng').length > 0) {
        const slider = document.getElementById('prc-rng')
    ...
    if ($('.rsnp-mnu, header.style5').length > 0) {
        const ps = new PerfectScrollbar('.rsnp-mnu, header.style5')
    }
...
$(window).on('load', function () {
    setTimeout(function () {
        $('.alert-success').eq(0).fadeOut(1000)
    }, 2000)
    ...
    if (jQuery('.fltr-itm').length > 0) {
        if (jQuery().isotope) {
            const jQuerycontainer = jQuery('.masonry, .msonry') // cache container
            const jQuerycontainer2 = jQuery('.msonry2') // cache container
